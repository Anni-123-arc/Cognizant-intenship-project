...
async getOrderDetails(orderID: string) {
  try {
    const response = await axios.get(`http://backend:3000//api/track/${orderID}`).then(res => res.data);
...
async getOrderHistory(email: string) {
   ...
   try {
    const response = await axios.get(`http://backend:3000/api/history/${email}`).then(res => res.data);
